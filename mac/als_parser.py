...
def scan_vst_plugins(vst2_names: list, vst3_names: list) -> dict:
    """
    Scan standard system VST/AU directories.
    Returns {("vst2"|"vst3"|"au", name): bool}.
    """
    dirs = _vst_dirs()
    v2_ext = (".dll",) if sys.platform != "darwin" else (".vst",)
    vst2_stems = _collect_stems(dirs["vst2"], v2_ext)
    vst3_stems = _collect_stems(dirs["vst3"], (".vst3",))
    au_stems   = _collect_stems(dirs.get("au", []), (".component",))
    result = {}
    for n in vst2_names:
        result[("vst2", n)] = _match_plugin(n, vst2_stems)
    for n in vst3_names:
        result[("vst3", n)] = _match_plugin(n, vst3_stems | au_stems)
    return result
